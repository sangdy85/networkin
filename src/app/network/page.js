...
export default function NetworkPage() {
  ...
  // Fetch from Real Backend SQLite API `/api/network`, `/api/auth/users`, `/api/clients`
  useEffect(() => {
    fetchItemsFromAPI();
    fetchUsersFromAPI();
    fetchClientsFromAPI();
  ...
  const handleSaveSubmit = async (e) => {
    ...
    const itemData = {
      ...
      site: formSite.trim(),
      workers: cleanWorkers.length > 0 ? cleanWorkers : [workerList[0] || '김철수 과장'],
            ...
            <form onSubmit={handleSaveSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.5rem', marginTop: '0.5rem' }}>
                ...
                <button type="submit" className="btn btn-accent" disabled={isUploading}>
                  {isUploading ? '업로드 및 저장 중...' : (editingItem ? '수정 완료' : '네트워크 작업 등록 (일정 자동연동)')}
                </button>
              </div>
            </form>
          </div>
              ...
              filteredItems.map(item => (
                <tr key={item.id}>
                  <td
                    ...
                    title="상세 내용 보기"
                  >
                    <span style={{ borderBottom: '1px dashed rgba(0,180,216,0.6)' }}>
                      {item.title}
                    </span>
                    {((item.files && item.files.length > 0) || item.filePath) && (
                      <span style={{ marginLeft: '0.4rem', fontSize: '0.75rem', background: 'rgba(0,180,216,0.2)', color: '#00B4D8', padding: '0.1rem 0.35rem', borderRadius: '4px' }}>
                        📎 {item.files?.length || 1}
                      </span>
                    )}
                  </td>
